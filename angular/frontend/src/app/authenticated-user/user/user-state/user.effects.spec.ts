...
fdescribe('UserEffects', () => {
    ...
    it('should call fetch the user profile if there is no user in state', 
        ...
        effects.loadUserProfile$.pipe(toArray()).subscribe({
    ...
    it('should submit the edited user data to backend and update user data in state', 
      ...
      effects.submitEditedProfile$.pipe(toArray()).subscribe({
        next: (actualActions2) => actualActions = actualActions2,
        error: fail,
      });
