...
fdescribe('monthlyTaskAppliedQuarterlysReducer', () => {
    ...
    it('returns the state with monthly task applied quarterly entities and indicates that ' 
        + 'the application objects have been loaded', () => {
        const state = monthlyTaskAppliedQuarterlysReducer(initialMonthlyTaskAppliedQuarterlysState, 
        new MonthlyTaskAppliedQuarterlysLoaded({ monthlyTasks: mTAQQuarter2Data }));
        expect(state).toEqual(statePriorToNewMTAQSubmission.monthlyTasksAppliedQuarterly);
    });


    it('returns the original state with one less monthly task applied quarterly  ' 
       + 'entity and indicates that the third monthly task scheduler has been sucessfully deleted', 
        () => {
        const state = monthlyTaskAppliedQuarterlysReducer(
            stateAfterNewMTAQSubmission.monthlyTasksAppliedQuarterly, 
        new MonthlyTaskAppliedQuarterlyDeletionSaved({ 
            id: 3, 
            message: stateFollowingNewMTAQDeletion.monthlyTasksAppliedQuarterly.successMessage 
        }));
        expect(state).toEqual(stateFollowingNewMTAQDeletion.monthlyTasksAppliedQuarterly);
    });
    ...
    + 'and indicates that the deletion of the third monthly task application failed', 
        ...
        expect(state).toEqual(
            stateFollowingNewMTAQDeletionFailure.monthlyTasksAppliedQuarterly
        );
    });

    it('returns the state with new monthly task applied quarterly entity and indicates that ' 
    ...
    it('returns the state with originally loaded monthly task applied quarterlys ' 
        + 'entity and indicates that submission of a new monthly task applied quarterly' 
       ...
       expect(state).toEqual(
        stateAfterNewMTAQSubmissionFailure.monthlyTasksAppliedQuarterly
        );
    });
